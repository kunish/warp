...
The implemented command surface contains exactly 75 typed, allowlisted actions, all default-authorized after exact-action credential validation. Close actions (`window.close`, `tab.close`, `pane.close`) flow through normal Warp close behavior, so existing app warnings for unsaved state, running processes, or shared sessions remain in effect. The Block, Auth, Drive, and History families are absent, and `input.insert` plus `input.replace` stage text without submitting it.
## Packaging model
...
2. Open **Settings > Scripting**. Local control is enabled by default. Verify that the Scripting toggle is set to **Enabled**. If it has been disabled, re-enable it. Enabling scripting allows for programmatic and agentic control of Warp; refer to the docs for more info.
...
- **Protected local state.** The authoritative Scripting setting uses platform secure storage, never imports a value from ordinary or private preferences, and defaults to enabled. On POSIX platforms, discovery records and broker sockets use owner-only permissions. On Windows, local-control publication remains disabled until equivalent ACL and broker protections are implemented.
...
- Non-close actions may reuse an unexpired exact-action credential. Close actions flow through normal Warp close pathways and may show existing app warnings.
