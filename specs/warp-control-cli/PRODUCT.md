...
The public catalog contains exactly **75 actions** organized around stable user-facing nouns. All 75 actions are default-authorized once the user enables Scripting. Close actions (`window.close`, `tab.close`, `pane.close`) flow through Warp's normal close behavior, so existing app warnings for unsaved state, running processes, or shared sessions remain in effect. `block.list` is intentionally absent from the catalog. Input-staging commands place text in the input buffer but never submit it.
