...
5. **App-side enforcement:** The running Warp app verifies the exact granted action, resolves targets deterministically, and dispatches close actions through normal Warp close behavior so existing app warnings remain in effect.
...
- Dispatch close actions (`window.close`, `tab.close`, `pane.close`) through normal Warp close behavior so existing app warnings remain in effect.
...
- Connect to the broker socket and request credentials for any of the 75 catalog actions.
...
- Normal Warp close warnings prevent unattended destructive closes.
...
- Normal Warp close warnings require human attention for destructive closes.
...
- The catalog contains exactly 75 actions, all default-authorized.
