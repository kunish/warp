...
- Close actions (`window.close`, `tab.close`, `pane.close`) flow through normal Warp close behavior so existing app warnings remain in effect.
...
- **Close behavior:** Close actions dispatch through normal Warp close pathways, so existing app warnings for unsaved state, running processes, or shared sessions remain in effect.
...
- **Action count:** Tests verify the catalog contains exactly 75 actions, all default-authorized.
...
- **Same-user residual risk:** The broker authenticates the OS user, not the calling application. Any process running as the same user can request credentials. Mitigated by: protected enablement, short expiry, exact-action grants, app-side revalidation, normal Warp close warnings for destructive actions.
