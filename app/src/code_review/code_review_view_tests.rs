...
use crate::code_review::diff_state::{
    DiffStateModel, FileDiff, FileDiffAndContent, GitFileStatus,
};
...
/// Builds a `FileDiffAndContent` mirroring the modified, non-binary file diffs
/// produced by `create_loaded_state_with_editors`.
fn modified_file_diff_and_content(file_path: &str, content: &str) -> FileDiffAndContent {
    FileDiffAndContent {
        file_diff: FileDiff {
...
fn take_single_file_state(app: &mut App, file_path: &str, content: &str) -> FileState {
    let editor = create_editor_with_content(app, content);
    let (window_id, _) = app.add_window(WindowStyle::NotStealFocus, |_| TestView);
    let mut loaded =
        create_loaded_state_with_editors(app, window_id, vec![(file_path.to_string(), editor)]);
    loaded
        .file_states
        .shift_remove(file_path)
        .expect("file state was just inserted")
}

/// Regression test for the diff "flashing" on refresh: when a full reload
/// carries the same diff for a file, its editor must be reused so its loaded
/// state is preserved. If the editor were rebuilt instead, `all_editors_loaded`
/// would report `false` and `render` would blank the already-visible content
/// back to the loading skeleton.
#[test]
fn test_reuse_file_state_preserves_loaded_editor_for_unchanged_file() {
    App::test((), |mut app| async move {
        let ctx = TestContext::new(&mut app, "test.txt", "line 1\nline 2\nline 3");

        let prev = take_single_file_state(&mut app, "src/main.rs", "line 1\nline 2\nline 3");
        assert!(
            prev.editor_state.as_ref().unwrap().is_loaded(),
            "precondition: the existing editor starts loaded"
        );
        let file = modified_file_diff_and_content("src/main.rs", "line 1\nline 2\nline 3");
        assert_eq!(
            prev.file_diff, file.file_diff,
            "precondition: the incoming diff is identical to the existing one"
        );
        ...
        ctx.code_review_view.update(&mut app, |view, view_ctx| {
            ...
            assert!(
                reused.editor_state.as_ref().unwrap().is_loaded(),
                "reusing an editor must preserve its loaded state so the view does \
                 not fall back to the loading skeleton"
            );
        });
    });
}
...
fn test_reuse_file_state_rebuilds_when_file_becomes_binary() {
    App::test((), |mut app| async move {
        ...
        let prev = take_single_file_state(&mut app, "asset.png", "x");
        let mut file = modified_file_diff_and_content("asset.png", "x");
        file.file_diff.is_binary = true;

        ctx.code_review_view.update(&mut app, |view, view_ctx| {
            assert!(
                view.reuse_file_state_if_compatible(prev, &file, view_ctx)
                    .is_none(),
                "a file that became binary can no longer reuse its text editor"
            );
        });
    });
}
...
#[test]
fn test_reuse_file_state_rebuilds_when_deleted_state_toggles() {
    App::test((), |mut app| async move {
        let ctx = TestContext::new(&mut app, "test.txt", "x");

        let prev = take_single_file_state(&mut app, "src/main.rs", "x");
        let mut file = modified_file_diff_and_content("src/main.rs", "x");
        file.file_diff.status = GitFileStatus::Deleted;
