...
fn handle_window_close(
    ...
    Ok(json!({ "action": "window.close", "ok": true }))
}
...
fn handle_tab_close(
    ...
    if closed {
        Ok(json!({ "action": "tab.close", "ok": true }))
    } else {
        Err(ControlError::new(
            ErrorCode::TargetStateConflict,
            "the tab close target changed before execution",
        ))
    }
}
...
fn handle_pane_close(
    ...
    let (pane_group, pane_id) = workspace.read(ctx, |workspace, ctx| {
