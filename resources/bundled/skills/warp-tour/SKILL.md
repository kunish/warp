...
If `warpctrl` is missing, no compatible instance is found, or the command reports that local control is disabled, print:
...
4. Before issuing close actions, use Ask User Question to tell the user that Warp will request one-shot in-app confirmation:
   - **Clean up tour panes/tabs**
   - **Leave them open and end**
5. If cleanup is chosen, issue exact-ID close commands and tell the user to approve each confirmation:
...
6. Never close the anchor or any target that existed before the tour. If confirmation is declined or cleanup fails, report exactly what remains open.
