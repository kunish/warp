...
fn response_error_serializes_machine_code() {
    let response = ResponseEnvelope::error(
        Uuid::nil(),
        ControlError::new(ErrorCode::MissingTarget, "no active window"),
    );
    let value = serde_json::to_value(&response).expect("response serializes");
    assert_eq!(value["response"]["status"], "error");
    assert_eq!(value["response"]["error"]["code"], "missing_target");
}
...
#[test]
fn all_75_actions_share_the_same_default_authorization_policy() {
